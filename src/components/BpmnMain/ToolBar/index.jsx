...
class BpmnToolBar extends Component {
  ...
  download = (type, data, name) => {
    ...
    name = name || `process_test.${dataTrack}`;
  ...
  onSave = async () => {
    try {
      const res = await this.props.bpmn.saveXML({format: true});
      await saveXml("process_test", "category_test", res.xml).then((d) => {
        message.success(d);
      });
      this.props.history.push("/flow/definition");
    } catch (e) {
